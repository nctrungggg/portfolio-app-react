...
const Skills = () => {
  return (
    <div name="skills" className="w-full h-screen  dark:text-gray-300 mb-36 ">
      {/* Container */}
      <div className="max-w-[1000px] mx-auto p-4 flex flex-col justify-center w-full h-full">
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-4 gap-4 text-center py-8">
          {skillList.map((skill) => (
            <div className="shadow-md shadow-slate-200 dark:shadow-[#040c16] hover:scale-110 duration-500">
              <img className="w-20 mx-auto" src={skill.img} alt={skill.alt} />
              <p className="font-medium my-4">{skill.title}</p>
            </div>
          ))}
        </div>
      </div>
